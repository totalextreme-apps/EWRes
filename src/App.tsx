...
type SkillsAnalysisViewMode = "default" | "breakdown" | "radar";
...
function SkillsAnalysisCombinedView(props: { points: SkillsAnalysisPoint[]; workerName: string }) {
  const { points, workerName } = props;
  const size = 280;
  const cx = size / 2;
  const cy = size / 2;
    ...
    >
      ...
      >
        <div>
          ...
          >
            <div style={{ fontSize: 14, fontWeight: 900, color: "rgba(255,255,255,0.96)" }}>
              Percentile vs Loaded Wrestlers
            </div>
            <div
              style={{
                fontSize: 11,
          ...
          <div style={{ display: "flex", justifyContent: "center" }}>
            ...
            >
              ...
              <polygon
                points={polygon}
                fill="rgba(235, 19, 48, 0.28)"
                stroke="rgba(255, 62, 87, 0.96)"
                strokeWidth="3"
              />
        ...
        >
          {points.map((point) => (
            <div
              key={`${point.label}-legend-combined`}
              style={{
                borderRadius: 12,
            ...
            >
              ...
              <div style={{ fontSize: 11, color: "rgba(255,255,255,0.72)", lineHeight: 1.1 }}>
      ...
      <div style={{ marginTop: 18 }}>
        ...
        <div style={{ display: "grid", gap: 10 }}>
          {points.map((point) => (
            ...
            >
              <div style={{ fontSize: 14, fontWeight: 900, color: "rgba(255,255,255,0.94)" }}>
                {point.label}
              </div>
              <div
                style={{
...
function SkillsAnalysisRadarChart(props: { points: SkillsAnalysisPoint[] }) {
  ...
  return (
    ...
    >
      ...
      <div style={{ display: "grid", gridTemplateColumns: "minmax(0,1fr) 156px", gap: 12, alignItems: "center" }}>
        <div style={{ display: "flex", justifyContent: "center" }}>
          <svg viewBox={`0 0 ${size} ${size}`} style={{ width: "100%", maxWidth: 500, height: "auto", overflow: "visible" }}>
            {rings.map((pct) => {
              const r = radius * (pct / 100);
              const pts = points
                .map((_, index) => {
        ...
        <div style={{ display: "grid", gap: 8 }}>
          {points.map((point) => (
            ...
            >
              ...
              <div style={{ fontSize: 11, color: "rgba(255,255,255,0.72)" }}>{point.percentile}th percentile</div>
            </div>
          ))}
        </div>
      </div>
...
export default function App() {
  ...
  const [filterRosterSplit, setFilterRosterSplit] = useState<string>("any");
  const [filterRosterPosition, setFilterRosterPosition] = useState<string>("any");
...
const [isSaveWorkspace, setIsSaveWorkspace] = useState<boolean>(false);
  ...
  const [saveContractFixStatusAction, setSaveContractFixStatusAction] = useState<"no_change" | "Available" | "Sitting Out Contract" | "In Rehab">("no_change");

  useEffect(() => {
    setSaveContractFixStatusAction("no_change");
  }, [selectedRecordIndex]);
  const [skillsAnalysisCollapsed, setSkillsAnalysisCollapsed] = useState(false);
  const [skillsAnalysisView, setSkillsAnalysisView] = useState<SkillsAnalysisViewMode>("default");
...
function clearDataFolder() {
  ...
  const rosterSplitFilterOptions = useMemo<string[]>(() => {
    if (typeof filterWorksFor !== "number") return [];
    const rec = employmentPromoRecords.find((r) => r.id === filterWorksFor);
    if (!rec) return [];
    return (rec.rosterSplits || [])
      .map((s) => String(s ?? "").trim())
      .filter((s) => s && s.toLowerCase() !== "none");
  }, [employmentPromoRecords, filterWorksFor]);

  const rosterPositionFilterOptions = useMemo(
    () => [
      { value: "any", label: "Any" },
      { value: "active_roster", label: "Active Roster Only" },
      { value: "main_event", label: "Main Eventers" },
      { value: "upper_midcard", label: "Upper Midcarders" },
      { value: "midcard", label: "Midcarders" },
      { value: "lower_midcard", label: "Lower Midcarders" },
      { value: "opener", label: "Openers" },
      { value: "jobber", label: "Jobbers" },
      { value: "developmental", label: "Developmental" },
      { value: "non_wrestler", label: "Non-Wrestlers" },
      { value: "manager", label: "Managers" },
      { value: "touring", label: "Touring Commitments" },
  ...
  const profileFilteredWorkers = useMemo(() => {
    ...
    if (filterRosterSplit !== "any" && typeof filterWorksFor === "number") {
      const wantPromo = Number(filterWorksFor) | 0;
      list = list.filter((w: any) => {
        return [1, 2, 3].some((slot) => {
          const promoId = getNum(w, `employer${slot}PromoId`) | 0;
          if (promoId !== wantPromo) return false;
          const roster = (getStr(w, `employer${slot}Roster`) || "None").trim() || "None";
          return roster === filterRosterSplit;
        });
      });
    }

    if (filterRosterPosition !== "any" && typeof filterWorksFor === "number") {
      const wantPromo = Number(filterWorksFor) | 0;
      list = list.filter((w: any) => {
        return [1, 2, 3].some((slot) => {
          const promoId = getNum(w, `employer${slot}PromoId`) | 0;
          ...
          const positionRaw = getNum(w, `employer${slot}PositionRaw`) | 0;
          const statusCode = String(getStr(w, "employmentStatusCode") || "Nor").trim() || "Nor";
          const touringCode = String(getStr(w, "touringCode") || "").trim();

          switch (filterRosterPosition) {
            case "active_roster":
              return positionRaw >= 1 && positionRaw <= 6;
            case "main_event":
              return positionRaw === 1;
            case "upper_midcard":
              return positionRaw === 2;
            case "midcard":
              return positionRaw === 3;
            case "lower_midcard":
              return positionRaw === 4;
            case "opener":
              return positionRaw === 5;
            case "jobber":
              return positionRaw === 6;
            case "developmental":
              return positionRaw === 7;
            case "non_wrestler":
              return positionRaw === 8;
            case "manager":
              return positionRaw === 9;
            case "touring":
              return !!touringCode;
            case "active":
              return statusCode === "Nor";
            case "inactive":
              return statusCode !== "Nor";
            default:
              return true;
    ...
    return list;
  }, [workers, filterNationality, filterGender, filterBirthMonth, filterWeight, filterSpeaks, filterPrimaryFinisherType, filterSecondaryFinisherType, filterWorksFor, filterRosterSplit, filterRosterPosition, filterContractType, filterTouringWith, filterAgeMin, filterAgeMax, filterWageMin, filterWageMax, flagFilters, skillRangeFilters]);
  ...
  const activeFilterCount = useMemo(() => {
    ...
    if (filterRosterSplit !== "any") n++;
    if (filterRosterPosition !== "any") n++;
  ...
  function clearAllFilters() {
    ...
    setFilterRosterSplit("any");
    setFilterRosterPosition("any");
    ...
    <div className={"ewr-filterPanel" + (compact ? " ewr-filterPanelCompact" : "")}>
              ...
              <div className="ewr-filterGrid">
                ...
                <div className="ewr-field">
                  ...
                  <EwrSelectCompat
                    className="ewr-input"
                    value={filterRosterSplit}
                    disabled={typeof filterWorksFor !== "number" || rosterSplitFilterOptions.length === 0}
                    onChange={(e) => setFilterRosterSplit(e.target.value)}
                  >
                    <option value="any">Any</option>
                    {rosterSplitFilterOptions.map((split) => (
                ...
                <div className="ewr-field">
                  <div className="ewr-label">Roster Position</div>
                  <EwrSelectCompat
                    className="ewr-input"
                    value={filterRosterPosition}
                    disabled={typeof filterWorksFor !== "number"}
                    onChange={(e) => setFilterRosterPosition(e.target.value)}
                  >
                    {rosterPositionFilterOptions.map((option) => (
              ...
              <div className="ewr-section">
                ...
                >
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap", marginLeft: "auto" }}>
                    <div className="ewr-field" style={{ minWidth: 220, margin: 0 }}>
                      <div className="ewr-label">View Option</div>
                      <EwrSelectCompat
                        className="ewr-input"
                        value={skillsAnalysisView}
                        onChange={(e) => setSkillsAnalysisView(e.target.value as SkillsAnalysisViewMode)}
                      >
                        <option value="default">Default</option>
                        <option value="breakdown">Skills Breakdown</option>
                        <option value="radar">Percentile Comparison</option>
                      </EwrSelectCompat>
                    </div>
                    <button
                      type="button"
                      ...
                      style={{ minWidth: 112, alignSelf: "flex-end" }}
                    >
                ...
                <div className="ewr-sectionBody">
                  ...
                  {(() => {
                    ...
                    const currentEmploymentStatusLabel = normalizedEmploymentStatusCode === "Hom"
                      ...
                      : normalizedEmploymentStatusCode === "Reh"
                        ? "In Rehab"
                            ...
                            <div style={{ display: "grid", gridTemplateColumns: "repeat(3, minmax(0, 1fr))", gap: 12, width: "100%" }}>
                              ...
                              <div className="ewr-field" style={{ marginBottom: 0 }}>
                                ...
                                <EwrSelectCompat
                                  ...
                                  onChange={(e) => {
                                    const action = e.target.value as "no_change" | "Available" | "Sitting Out Contract" | "In Rehab";
                                    setSaveContractFixStatusAction(action);
                                    if (action === "no_change") return;
                                    const next = action === "Sitting Out Contract" ? "Hom" : action === "In Rehab" ? "Reh" : "Nor";
                                    updateSelected({
                                ...
                                >
                                  ...
                                  <option value="In Rehab">Set to In Rehab</option>
                                </EwrSelectCompat>
